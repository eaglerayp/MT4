...
extern int stp = 800;
extern double prestage_stp = 100;
...
const double MinProfit = -99999999999;
...
double profitMap[100];

int OnInit()
{
//---
	magicnum = Month() * 100 + Day();
	codeversion = "20160705 AUDUSD";
	...
	if ((!offquote_error) && close_fail == 0) {
		...
		if (reverse_event) {
			...
			if (stage_code == 1) { //pre stage
				...
				printf("this ann lots size:" + announce_lots_size);
...
void checkOrderTime() {
	...
	for (int i = 0; i < total; i++) {
		...
		if (select) {
			...
			int index = Hashindex(magic);
			if (profit > profitMap[index]) {
				profitMap[index] = profit;
			}
		...
		if (select) {
			...
			if (sym_close == sym && currentTime > Orderopen + five_mins) {
				if (OrderType() == OP_BUY && Bid < stoploss_value) {
					...
					if (InPostStage) {
						reverse_event = true;
						Sell(sym, lots_size, "reverse Sell", OrderMagicNumber(), big_slip, reverse_stage);
					...
					if (InPostStage) {
						reverse_event = true;
						Buy(sym, lots_size, "reverse Buy", OrderMagicNumber(), big_slip, reverse_stage);
						printf("close by stoploss, big slip");
					}
				}
			}
		...
		if (select) {
			...
			if (sym_close == sym) {
				...
				if (!close) {
					...
					int magic = OrderMagicNumber();
					int index = Hashindex(magic);
					printf("close magic: " + magic + "; maxprofit in duration: " + profitMap[index]);
				}
			}
		}//end select
	}//end for
